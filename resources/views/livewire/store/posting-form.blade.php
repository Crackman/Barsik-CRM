<div class="row">
  <form wire:submit.prevent="save" id="posting">
    ...
    <table class="table table-sm">
      ...
      <tr>
        <td>
          <button class="btn btn-success btn-sm btn-plus" wire:click="$emitTo('store.product-table', 'setProductsCount', {{ $products_count  }})"
      ...
      @if (!@empty($approved_products))
        @foreach ($approved_products as $product)
          <tr>
            ...
            <td>
              {{ ($product->purchase_price * $product->count) ?? ' ' }}
            </td>
            <td>
            ...
            <td>
              <button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#product-form-modal"
                      wire:click="$emitTo('store.product-form','triggerEditProduct', {{ $product }})">
